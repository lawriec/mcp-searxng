...
Route searches through different countries simultaneously. Each region gets its own
VPN+SearXNG container pair, and the MCP server gains a `region` parameter to target searches
geographically. This is especially powerful for foreign-language research — a Japanese VPN
exit surfaces different Google results than a UK one.

**1. Organize profiles into subfolders** by region:

```
vpn-profiles/
  default.auth          # shared auth — inherited by all subfolders
  uk/
    uk-server1.ovpn
    uk-server2.ovpn
  jp/
...
Auth files are inherited: subfolders without their own `.auth` file use the root
`default.auth`. Per-subfolder auth works the same way as the single-VPN setup.

**2. Generate the Docker config:**

```bash
cd docker
./setup-regions.sh ~/vpn-profiles
```

This creates `docker/generated/` with a compose file and per-region SearXNG settings.
Each region's SearXNG listens on its own port (8081, 8082, ...).

**3. Set the regions env var** (printed by the setup script):

```bash
export SEARXNG_REGIONS="uk=http://localhost:8081,jp=http://localhost:8082,us=http://localhost:8083"
```

**4. Start the region containers:**

```bash
cd docker
docker compose -f docker-compose.yml -f generated/docker-compose.regions.yml up -d
```

Or combine with single-VPN for the main instance plus regions:

```bash
docker compose -f docker-compose.yml -f docker-compose.vpn.yml -f generated/docker-compose.regions.yml up -d
```

**5. Use from the MCP server:**

```
# Search via Japanese VPN exit
searxng_search(query: "幻のアニメ", region: "jp", language: "ja")

# Search via UK VPN exit
searxng_search(query: "lost BBC programme", region: "uk")

# List available regions
searxng_vpn_regions()
```

The `region` parameter and `searxng_vpn_regions` tool only appear when `SEARXNG_REGIONS`
is set. Without it, everything works exactly as before.

Each region's VPN container auto-rotates profiles within its subfolder (controlled by
`ROTATE_INTERVAL_MINS`, default 30 min), so IPs change but stay within the same country.

**Coexistence with single-VPN**: Flat `.ovpn` files at the root of your config dir still
work for the single-VPN setup. Subfolders enable multi-region. Both can coexist — the main
SearXNG uses the flat files, region containers use the subfolders.
...
| `region` | string | No | VPN exit region (e.g. `"uk"`, `"jp"`). Only available when `SEARXNG_REGIONS` is set. See [Multi-Region VPN](#multi-region-vpn). |
...
| `SEARXNG_REGIONS` | No | — | Multi-region mapping: `uk=http://localhost:8081,jp=http://localhost:8082`. Generated by `setup-regions.sh`. |
